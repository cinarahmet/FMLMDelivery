...
namespace FMLMDelivery
{
    class Runner
    {
        ...
        private Dictionary<string,Dictionary<string,Double>> _real_distances;

        public Runner(List<DemandPoint> _counties, List<xDocks> _xDocks, List<xDocks> _agency, List<Seller> small_seller, List<Seller> big_seller, List<Seller> total_seller, Dictionary<string, Dictionary<string, Double>> real_distances)
        {
            xDocks = _xDocks;
            county = _counties;
            agency = _agency;
            ...
            _real_distances = real_distances;
        }
        ...
        public Tuple<List<xDocks>,List<Hub>> Run()
        {
            ...
            var p = 0;
            var first_phase = new DemandxDockModel(county, xDocks,_small_seller, demand_weighted_model, min_model_model, demand_covarage,_real_distances, phase_2, p);
            ...
            phase_2 = true;
            first_phase = new DemandxDockModel(county, xDocks, _small_seller, demand_weighted_model, min_model_model, demand_covarage,_real_distances, phase_2, min_num);
            first_phase.Run();
            objVal = first_phase.GetObjVal();
            new_xDocks = first_phase.Return_XDock();
            ...
            demand_covarage = 0.99;
            min_model_model = true;
            demand_weighted_model = false;
            phase_2 = false;

            var second_phase = new xDockHubModel(new_xDocks, potential_Hubs, _total_seller, demand_weighted_model, min_model_model, demand_covarage,_real_distances, phase_2, p);
            second_phase.Run();
            var num_clusters = second_phase.Return_num_Hubs();
            min_model_model = false;
            demand_weighted_model = true;
            phase_2 = true;
            second_phase = new xDockHubModel(new_xDocks, potential_Hubs, _total_seller, demand_weighted_model, min_model_model, demand_covarage,_real_distances, phase_2, num_clusters);
            second_phase.Run();
